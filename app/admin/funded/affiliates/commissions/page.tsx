...
function pickRows(payload: unknown): Commission[] {
  const p = payload as any;

  if (Array.isArray(p)) return p;
  if (Array.isArray(p?.commissions)) return p.commissions;
  if (Array.isArray(p?.affiliateCommissions)) return p.affiliateCommissions;
...
export default function AdminCommissionReviewPage() {
  ...
  async function load() {
    ...
    const result = await adminGet<unknown>("admin/affiliate/commissions");
  ...
  async function action(id: string, actionName: "approve" | "reject") {
    ...
    const result = await adminPost(
      `admin/affiliate/commissions/${id}/${actionName}`,
      {}
    );
      ...
      <section className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold">Commission Review</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-400">
              Approve or reject affiliate commissions before they become payable.
            </p>
          </div>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex justify-end gap-2">
                          <button
                            disabled={
                              busyId === id ||
                              ["approved", "rejected", "paid", "payout_requested"].includes(
                                String(row.status)
                              )
                            }
                            ...
                            className={
                              row.status === "approved"
                                ...
                                : ["rejected", "paid", "payout_requested"].includes(
                                      String(row.status)
                                    )
                          ...
                          <button
                            ...
                            className={
                              row.status === "rejected"
                                ...
                                : ["approved", "paid", "payout_requested"].includes(
                                      String(row.status)
                                    )
                                  ? "cursor-not-allowed rounded-lg bg-slate-800 px-3 py-2 text-xs font-bold text-slate-500 opacity-60"
